...
export function getAffordableItems(items, maxPrice) {
  // Your code here
  if (!Array.isArray(items) || typeof maxPrice === "string") {
    return [];
  }
...
const formatedBill = items.map((item)=> {
  const singlePrice = `${item.name} x ${item.qty} = Rs.${item.price * item.qty }`;
  return singlePrice
})
const result = formatedBill.join("\n")
return result;
}
